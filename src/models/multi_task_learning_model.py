...
class Multi_Task_Learning_Model(nn.Layer):
    """
    ...
    def __init__(self, model, tasks, num_events, weights_events, input_size, sigma, thresh_ball_pos_mask, device):
        ...
        self.log_vars = self.create_parameter(shape=(self.num_tasks,), default_initializer=Assign(paddle.zeros((self.num_tasks,))))
        self.w = input_size[0]
        self.h = input_size[1]
        self.sigma = sigma
    ...
    def forward(self, resize_batch_input, org_ball_pos_xy, global_ball_pos_xy, target_events, target_seg):
        ...
        total_loss = global_ball_loss / (paddle.exp(2 * self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
        ...
        if pred_ball_local is not None:
            ...
            total_loss += local_ball_loss / (paddle.exp(2 * self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
        ...
        if pred_events is not None:
            ...
            total_loss += event_loss / (2 * paddle.exp(self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]

        if pred_seg is not None:
            log_vars_idx += 1
            seg_loss = self.seg_loss_criterion(pred_seg, target_seg)
            total_loss += seg_loss / (2 * paddle.exp(self.log_vars[log_vars_idx])) + self.log_vars[log_vars_idx]
